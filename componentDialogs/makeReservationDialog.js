...
class MakeReservationDialog extends ComponentDialog {
    ...
    async getName(step) {
        ...
        if (step.result === true) {
            ...
            await step.context.sendActivity('You chose to cancel the reservation');
            endDialog = true;
    ...
    async getNumberOfParticipants(step) {
        ...
        if (!step.values.noOfParticipants) {
            return await step.prompt(NUMBER_PROMPT, 'How many participants ( 1 - 10)?');
        } else {
    ...
    async confirmStep(step) {
        ...
        return await step.prompt(
            CONFIRM_PROMPT,
            'Are you sure that all values are correct and you want to make the reservation?',
            ['yes', 'no']
        );
    }
